      ...
      <div class="rule-list-container">
        ...
        <el-dialog :visible.sync="dialogTemplateVisible" :show-close="false">
          <div class="rule-template-dialog">
            ...
            <el-form ref="form" :model="dialogTemplateForm" label-width="100px" size="small">
              <el-form-item label-width="0">
                <div class="template-table">
                  <pagination-section :page-data="dialogTemplatePageData" @handleSizeChange="handleTemplateSizeChange" @handleCurrentChange="handleTemplateCurrentChange">
                    <el-table
                      ref="multipleTable"
                      :data="dialogTemplateTableData"
                      style="width: 100%"
                      max-height="240px"
                      size="small"
                    >
...
export default {
  ...
  data() {
    return {
      ...
      dialogTemplateForm: {},
      dialogTemplatePageData: {
        index: 1,
        size: 10,
        total: 0
      },
      dialogTemplateTableData: []
    }
  },
  created() {
  ...
  methods: {
    ...
    leadInAction() {
      this.getPlatformRuleTemplateList((data) => {
        if (data && data.length) {
    ...
    getPlatformRuleTemplateList(callback) {
      ruleTemplateList({ ...this.pageData, type: this.form.type, sponsorId: 0 }).then(response => {
        const { data, total } = response.data
        this.dialogTemplatePageData.total = total
        this.dialogTemplateTableData = data
        callback && callback(data)
      })
    },
    indexPlatformMethod(indexIndex) {
      const { index, size } = this.dialogTemplatePageData
      return (index - 1) * size + (indexIndex + 1)
    },
    templateSelectedAction(scope) {
      ...
      this.dialogTemplatePageData = {
        index: 1,
        size: 10,
        total: 0
      }
      this.dialogTemplateTableData = []
    },
    handleTemplateSizeChange(value) {
      this.dialogTemplatePageData.size = value
      this.getPlatformRuleTemplateList()
    },
    handleTemplateCurrentChange(value) {
      this.dialogTemplatePageData.index = value
      this.getPlatformRuleTemplateList()
    }
  }
}
