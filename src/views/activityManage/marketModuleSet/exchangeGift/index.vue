<template xmlns:v-slot="http://www.w3.org/1999/XSL/Transform">
  <layout-sub-child :route-name="routeName">
    <template v-slot:default>
      <div class="exchange-gift-container">
        <el-row class="exchange-gift-wrap" type="flex" justify="space-between" align="middle">
...
import { ruleData, exchangeAndPrizeGiftList, exchangeAndPrizeGiftDelete } from '@/api/activityManage/marketModuleSet'
...
export default {
  ...
  data() {
    return {
      routeName: 'ExchangeGift',
      loading: false,
      ruleType: 'point_shop_rule',
      dialogTemplateVisible: false,
      ruleCont: '',
      setPathData: '/activityManage/marketModuleSet/exchangeGift/giftSet',
      pageData: {
        index: 1,
        size: 10,
        total: 0
  ...
  created() {
    if (this.$route.name === this.routeName) {
      this.getRuleData()
      this.getExchangeAndPrizeGiftList()
    }
  },
  methods: {
    getRuleData() {
    ...
    getExchangeAndPrizeGiftList() {
      exchangeAndPrizeGiftList({ ...this.pageData, activityId: this.activityData.id, giftType: '0' }).then(response => {
