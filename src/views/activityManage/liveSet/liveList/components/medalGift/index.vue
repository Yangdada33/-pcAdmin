<template>
  <div class="medal-gift-container">
    <div class="gift-wrap">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px" size="small">
        ...
        <el-form-item label="领取规则：" prop="ruleCont" class="rule-cont clearfix">
          <el-row type="flex">
            <el-input v-model="form.ruleCont" :rows="5" type="textarea" style="width: 500px" placeholder="请输入领取规则" />
        ...
        <el-form-item label="领取条件：" prop="medalNum">
          ...
          <div class="form-tip">注：需要直播章个数</div>
        </el-form-item>
...
export default {
  ...
  methods: {
    getMedalGiftData() {
      medalGiftData({ activityId: this.activityData.id, medalType: 1 }).then(response => {
    ...
    formSaveAction() {
      this.$refs.form.validate(valid => {
        if (valid) {
          this.loading = true
          medalGiftSet({ activityId: this.activityData.id, ...this.form, medalType: 1 }).then(response => {
            this.$message.success(response.msg)
            this.loading = false
    ...
    getRuleTemplateList() {
      return new Promise((resolve, reject) => {
        ruleTemplateList({ ...this.templatePageData, type: ruleTypeData.liveGift }).then(response => {
